...
- [completed] 1. 映射代理订阅、规则订阅的私有存储、刷新入口与 GPUI 生命周期
- [completed] 2. 以 TDD 锁定更新间隔持久化、旧数据兼容、立即刷新和过期任务隔离
...
- [completed] 1. 盘点所有面向用户的时间展示点与日志级别渲染
- [completed] 2. 以 TDD 建立本地偏移格式化与级别到颜色的映射
- [completed] 3. 新增主题琥珀色并接入内核与 UI 两个日志列表
- [completed] 4. 完成对比度校验、原生截图、全仓验证与 Git 提交

## 状态信息重复修复

- [completed] 1. 定位侧边栏与状态栏两处重复的控制器摘要
- [completed] 2. 以测试锁定状态行必须使用真实内核名而非硬编码
- [completed] 3. 删除侧边栏副本，并把失败原因并入状态栏避免丢失
- [completed] 4. 完成全仓验证、视觉确认与 Git 提交
...
| 更新计划时把 progress 文本误放进 task_plan 补丁上下文 | 1 | 拆成两个精确文件补丁后成功，不重复使用混合上下文 |
...
| 给 `Theme` 加第 17 个颜色后，87 处按值传参触发 Clippy 的 256 字节上限 | 1 | 不做 87 处 `&Theme` 无关重构；`Theme` 是刻意按值传递的 Copy 令牌集，改为在 `clippy.toml` 写明理由并把 `pass-by-value-size-limit` 提到 512 |
| 设计的「取不到时区就回退 UTC 并标注」无法实现 | 1 | chrono 的 `Local` 在系统时区不可读时静默回退到 UTC(0)，调用方无从区分；删掉伪保护与死分支，不留假装能检测失败的代码 |
