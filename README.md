...
Manis 会在启动时及每小时自动检查新版本，只读取版本信息，不下载或安装应用更新。
在「配置 → 应用更新」（配置页最底部）查看结果，点击「前往 GitHub」后自行下载和安装。
检查失败时，该按钮仍可使用。这里也显示当前版本和 GitHub 项目地址；托盘菜单的
「关于 Manis」会打开同样包含版本与项目地址的弹窗。Mihomo 内核的手动更新不受影响。
...
内核更新功能负责下载、校验、版本验证、原子替换和失败回滚。

未添加节点时，Manis 会准备一个只含 `DIRECT` 兜底的空配置；添加订阅或单独节点后，配置由
Manis 校验并写入私有运行目录。controller endpoint 也由 Manis 分配，不作为用户配置项。

来源和分流规则的修改先在私有暂存目录完成，再提交本次变更的文件；保存或应用失败不会整目录回滚其他设置。恢复时若发现同一文件已被再次修改，会保留新内容并报告恢复失败。应用配置前会先关闭当前代理并恢复相关系统设置，清理失败则保留当前配置并报告失败。
