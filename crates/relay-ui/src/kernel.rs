...
impl KernelRuntime {
    ...
    #[must_use]
    pub(crate) fn configured(store_dir: Option<&Path>) -> Self {
        let kind = store_dir
            .and_then(|directory| load_kernel_kind_in(directory).ok())
            .unwrap_or_default();
        match Self::prepare(kind, store_dir) {
            Ok(runtime) => runtime,
            Err(message) => Self {
                kind,
    ...
    pub(crate) fn prepare(kind: KernelKind, store_dir: Option<&Path>) -> Result<Self, String> {
        let controller = match kind {
            KernelKind::Mihomo => mihomo::configured_runtime(),
            KernelKind::SingBox => {
                let store_dir =
                    store_dir.ok_or_else(|| "无法确定来源目录，不能准备 sing-box".to_owned())?;
    ...
    pub(crate) fn initial_status(&self) -> String {
        match &self.controller {
            ControllerRuntime::External { .. } => {
                format!("演示数据 · 尚未连接 {}", self.kind.display_name())
            }
            ControllerRuntime::Managed { .. } => {
                format!("托管内核已配置 · 点击启动 {}", self.kind.display_name())
            }
            ControllerRuntime::Invalid { message } => {
                format!("{} 配置无效：{message}", self.kind.display_name())
            }
        }
    }
    ...
    pub(crate) fn button_label(&self, state: &ControllerState) -> &'static str {
        match (self.kind, &self.controller, state) {
            (_, _, ControllerState::Connecting { .. }) => "正在连接…",
            (KernelKind::Mihomo, ControllerRuntime::Managed { .. }, _) => "启动 Mihomo",
            (KernelKind::SingBox, ControllerRuntime::Managed { .. }, _) => "启动 sing-box",
            (_, _, ControllerState::Connected { .. }) => "刷新数据",
            (KernelKind::Mihomo, _, _) => "连接 Mihomo",
            (KernelKind::SingBox, _, _) => "连接 sing-box",
        }
    }
}
...
impl std::fmt::Display for KernelSelectionError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::Unavailable => "无法读取或保存内核选择",
            Self::UnsafeFile => "内核选择文件不是安全的普通文件",
            Self::InvalidValue => "内核选择包含未知值",
        })
    }
}
