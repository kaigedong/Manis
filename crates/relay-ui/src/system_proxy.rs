...
const RECOVERY_VERSION: &str = "relay-system-proxy-v1";
    ...
    default_relay_data_dir()
        ...
        .ok_or_else(|| {
            SystemProxyError::Unavailable(
                language
                    .text(
                        "Could not determine Relay data directory for system proxy recovery",
                        "无法确定 Relay 系统代理恢复目录",
                    )
                    .to_owned(),
            )
...
fn recovery_read_error(language: Language) -> SystemProxyError {
    SystemProxyError::CommandFailed(
        language
            .text(
                "Could not safely read Relay system proxy recovery snapshot",
                "无法安全读取 Relay 系统代理恢复快照",
            )
            .to_owned(),
    )
}
...
fn write_recovery_snapshot_at(
    ...
    let Some(directory) = path.parent() else {
        return Err(SystemProxyError::Unavailable(
            language
                .text(
                    "Could not determine Relay system proxy recovery directory",
                    "无法确定 Relay 系统代理恢复目录",
                )
                .to_owned(),
        ));
    ...
    let mut file = fs::OpenOptions::new()
        ...
        .map_err(|_error| {
            SystemProxyError::CommandFailed(
                language
                    .text(
                        "Could not create Relay system proxy recovery snapshot",
                        "无法创建 Relay 系统代理恢复快照",
                    )
                    .to_owned(),
            )
    ...
    file.set_permissions(fs::Permissions::from_mode(0o600))
        .map_err(|_error| {
            SystemProxyError::CommandFailed(
                language
                    .text(
                        "Could not protect Relay system proxy recovery snapshot",
                        "无法保护 Relay 系统代理恢复快照",
                    )
                    .to_owned(),
            )
    ...
    file.write_all(contents.as_bytes()).map_err(|_error| {
        SystemProxyError::CommandFailed(
            language
                .text(
                    "Could not write Relay system proxy recovery snapshot",
                    "无法写入 Relay 系统代理恢复快照",
                )
                .to_owned(),
        )
    ...
    file.sync_all().map_err(|_error| {
        SystemProxyError::CommandFailed(
            language
                .text(
                    "Could not flush Relay system proxy recovery snapshot",
                    "无法刷写 Relay 系统代理恢复快照",
                )
                .to_owned(),
        )
    ...
    fs::rename(&temporary, path).map_err(|_error| {
        ...
        SystemProxyError::CommandFailed(
            language
                .text(
                    "Could not replace Relay system proxy recovery snapshot",
                    "无法替换 Relay 系统代理恢复快照",
                )
                .to_owned(),
        )
...
fn prepare_recovery_directory(
    ...
    match fs::symlink_metadata(directory) {
        Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_dir() => {
            return Err(SystemProxyError::CommandFailed(
                language
                    .text(
                        "Relay system proxy recovery directory is unsafe",
                        "Relay 系统代理恢复目录不安全",
                    )
                    .to_owned(),
            ));
        ...
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            fs::create_dir_all(directory).map_err(|_error| {
                SystemProxyError::CommandFailed(
                    language
                        .text(
                            "Could not create Relay system proxy recovery directory",
                            "无法创建 Relay 系统代理恢复目录",
                        )
                        .to_owned(),
                )
        ...
        Err(_error) => {
            return Err(SystemProxyError::CommandFailed(
                language
                    .text(
                        "Could not inspect Relay system proxy recovery directory",
                        "无法检查 Relay 系统代理恢复目录",
                    )
                    .to_owned(),
            ));
    ...
    fs::set_permissions(directory, fs::Permissions::from_mode(0o700)).map_err(|_error| {
        SystemProxyError::CommandFailed(
            language
                .text(
                    "Could not protect Relay system proxy recovery directory",
                    "无法保护 Relay 系统代理恢复目录",
                )
                .to_owned(),
        )
...
fn delete_recovery_snapshot_at(path: &Path, language: Language) -> Result<(), SystemProxyError> {
    match fs::remove_file(path) {
        ...
        Err(_error) => Err(SystemProxyError::CommandFailed(
            language
                .text(
                    "Could not remove Relay system proxy recovery snapshot",
                    "无法删除 Relay 系统代理恢复快照",
                )
                .to_owned(),
        )),
...
mod macos {
    ...
    pub(super) fn recover_stale(language: Language) -> Result<(), SystemProxyError> {
        ...
        let snapshots = decode_snapshots(&contents).ok_or_else(|| {
            SystemProxyError::CommandFailed(
                language
                    .text(
                        "Relay system proxy recovery snapshot is invalid",
                        "Relay 系统代理恢复快照无效",
                    )
                    .to_owned(),
            )
    ...
    fn decode_snapshots(contents: &str) -> Option<Vec<ServiceSnapshot>> {
        ...
        (lines.next()? == RECOVERY_VERSION).then_some(())?;
    ...
    mod tests {
        ...
        fn macos_enable_writes_recovery_before_applying_proxy() {
            let root = test_directory("relay-system-proxy-enable");
        ...
        fn macos_failed_apply_keeps_recovery_when_rollback_also_fails() {
            let root = test_directory("relay-system-proxy-failed-rollback");
            let recovery = root.join("system-proxy.recovery");
            let mut runner = fake_macos_runner();
        ...
        fn macos_recover_stale_restores_snapshot_and_removes_marker() {
            let root = test_directory("relay-system-proxy-recover");
            let recovery = root.join("system-proxy.recovery");
            let snapshots = vec![ServiceSnapshot {
                name: "Wi-Fi".to_owned(),
                web: ProxySetting {
...
mod tests {
    ...
    fn recovery_reader_rejects_symbolic_links() {
        ...
        let root = std::env::temp_dir().join(format!(
            "relay-system-proxy-symlink-{}-{nonce}",
            std::process::id()
        ));
        std::fs::create_dir_all(&root).expect("create fixture directory");
        let target = root.join("target");
        let link = root.join("system-proxy.recovery");
        std::fs::write(&target, "relay-system-proxy-v1\nplatform=macos\n")
