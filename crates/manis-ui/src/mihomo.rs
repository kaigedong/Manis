...
const STORED_SUBSCRIPTION_VERSION: &str = "manis-subscription-source-v1";
...
const NODE_POLICY_GROUP_VERSION: &str = "manis-node-group-v1";
const LEGACY_RELAY_NODE_POLICY_GROUP_VERSION: &str = "relay-node-group-v1";
...
fn decode_node_policy_group(
    ...
    if !storage_version_supported(
        lines.next(),
        NODE_POLICY_GROUP_VERSION,
        LEGACY_RELAY_NODE_POLICY_GROUP_VERSION,
    ) {
        return Err(SubscriptionStoreError::StoredSourceUnavailable);
    }
    let mut id = None;
...
mod tests {
    ...
    fn legacy_relay_storage_versions_remain_readable() {
        ...
        assert!(super::storage_version_supported(
            Some(super::LEGACY_RELAY_NODE_POLICY_GROUP_VERSION),
            super::NODE_POLICY_GROUP_VERSION,
            super::LEGACY_RELAY_NODE_POLICY_GROUP_VERSION,
        ));
    }
...
";
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let root = test_temp_dir("manis-node-policy-groups");
        let store = root.join("subscriptions");
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        use manis_core::{NodeGroupMatcher, NodeGroupStrategy, NodeIdentity, NodePolicyGroup};
...
"#;
    ...
    #[test]
    fn external_runtime_keeps_manis_node_groups_local_only()
    -> Result<(), Box<dyn std::error::Error>> {
        use manis_core::RoutingMode;
        ...
        assert_eq!(runtime.load_node_group_runtime("Manis Group")?, None);
        assert!(
            runtime
                .select_node_group_node("Manis Group", "Candidate")
                .is_err()
        );
