    ...
    T: ControllerTransport,
{
    ...
    /// Replaces Mihomo's complete runtime configuration from an in-memory payload.
    ///
    /// The forced `PUT /configs` path matches Mihomo's full reload lifecycle: it suspends the
    /// tunnel and rebuilds general settings, DNS, listeners, TUN and providers in dependency
    /// order. The payload can contain credentials and must not be logged by callers.
    ///
    /// # Errors
    /// Returns an error if the controller rejects the configuration or the request fails.
    pub fn reload_config_payload(&self, payload: &str) -> Result<(), MihomoError> {
        self.transport.put_json(
            &self.config,
            "/configs?force=true",
    ...
    pub fn set_tun_enabled(&self, enabled: bool) -> Result<(), MihomoError> {
        let tun = if enabled {
            ...
            tun.insert(
                "dns-hijack".to_owned(),
                serde_json::json!(["any:53", "tcp://any:53"]),
            );
            tun
        } else {
        ...
        let patch = Map::from_iter([("tun".to_owned(), Value::Object(tun))]);
        self.transport
