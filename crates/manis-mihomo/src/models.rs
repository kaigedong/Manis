...
pub struct ConnectionMetadata {
    ...
    #[serde(rename = "sniffHost", default, alias = "sniff_host")]
    ...
    #[serde(rename = "remoteDestination", default, alias = "remote_destination")]
...
impl ConnectionMetadata {
    /// Returns the most informative target name reported by Mihomo.
    ///
    /// Mihomo serializes unavailable string fields as `""`, so every candidate must be trimmed
    /// before falling back. A sniffed hostname is more precise for display than the original host,
    /// followed by the destination IP and the remote address reported by the outbound adapter.
    #[must_use]
    pub fn target_host(&self) -> Option<&str> {
        [
            self.sniff_host.as_deref(),
            self.host.as_deref(),
...
impl ObservedRouteEvidence {
    fn from_connection(connection: &Connection) -> Self {
        Self {
            host: connection.metadata.target_host().map(str::to_owned),
