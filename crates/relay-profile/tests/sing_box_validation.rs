...
#[test]
#[ignore = "requires RELAY_SING_BOX_TEST_BINARY pointing to a local sing-box executable"]
fn generated_reality_profile_passes_sing_box_check() -> Result<(), Box<dyn std::error::Error>> {
    let binary = std::env::var_os("RELAY_SING_BOX_TEST_BINARY")
        .ok_or("RELAY_SING_BOX_TEST_BINARY is required")?;
    let root = std::env::temp_dir().join(format!(
        "relay-profile-sing-box-validation-{}",
        std::process::id()
    ));
    ...
    let config = write_private_atomic(&root, "relay-generated.json", json.as_bytes())?;
